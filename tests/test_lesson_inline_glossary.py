...
class LessonInlineGlossaryTests(unittest.TestCase):
    ...
    def test_highlighted_fixture_uses_only_first_safe_canonical_occurrences(self) -> None:
        ...
        self.assertIn(
            'data-bms-glossary-slug="anchor">Holding Point</a>',
            result.stdout,
        )
        self.assertIn('href="/learn/glossary/#ace"', result.stdout)
        self.assertIn('href="/learn/glossary/#anchor"', result.stdout)
        self.assertIn("Ace appears again, and anchor appears again", result.stdout)
    ...
    def test_excluded_contexts_and_visible_wording_remain_unchanged(self) -> None:
        ...
        self.assertIn("<code>anchor in inline code</code>", result.stdout)
        self.assertIn("Ace and Holding Point in a fenced code block.", result.stdout)
        self.assertIn('class="math inline"', result.stdout)
        self.assertIn('class="fixture-raw">Ace and anchor in raw HTML.</span>', result.stdout)
        self.assertIn('alt="Ace and anchor in a caption."', result.stdout)
    ...
    def test_control_lesson_has_no_inline_markup(self) -> None:
        ...
        self.assertIn("Ace and Holding Point appear in ordinary prose", result.stdout)
    ...
    def test_broad_terms_still_drive_all_backlinks(self) -> None:
        lesson = {
            ...
            "categories": ["Beginner"],
            "tags": ["Checker Play"],
            "terms": ["ace", "one-point", "anchor", "active-builder"],
            "highlighted_terms": ["ace", "anchor"],
            "title": "Fixture",
        }
        related = learn_glossary.validate_lessons([lesson], self.entries)
        self.assertEqual(
            set(related),
            {"ace", "one-point", "anchor", "active-builder"},
        )
        self.assertEqual(lesson["terms"], ["ace", "one-point", "anchor", "active-builder"])
        self.assertEqual(lesson["highlighted_terms"], ["ace", "anchor"])
            ...
            (
                {
                    **base,
                    "terms": ["anchor"],
                    "highlighted_terms": ["holding-point"],
                },
                "uses alias slug",
            ),
            (
                {**base, "highlighted_terms": ["anchor"]},
                "missing from terms",
            ),
        )
...
"""
    ...
    def test_generated_lookup_always_has_canonical_short_definitions(self) -> None:
        ...
        for slug in ("ace", "one-point", "anchor", "active-builder"):
            self.assertIsInstance(by_slug[slug]["short_definition"], str)
            self.assertTrue(by_slug[slug]["short_definition"])
        self.assertEqual(by_slug["anchor"]["aliases"], ["Holding Point"])
        self.assertEqual(by_slug["anchor"]["alias_slugs"], ["holding-point"])
    ...
    def test_exactly_one_real_lesson_has_highlighted_terms(self) -> None:
    ...
    def test_real_lesson_marks_only_two_first_safe_occurrences(self) -> None:
        source_path = (
            ROOT
            / "site"
            / "learn"
        ...
        self.assertEqual(result.stdout.count('class="bms-inline-glossary"'), 2)
        self.assertIn(
            'data-bms-glossary-slug="doubling-cube">cube</a>',
            result.stdout,
        )
        self.assertIn(
            'data-bms-glossary-slug="take">take</a>',
            result.stdout,
        )
        self.assertEqual(
            result.stdout.count('data-bms-glossary-slug="doubling-cube"'),
            1,
        )
        self.assertEqual(
            result.stdout.count('data-bms-glossary-slug="take"'),
            1,
        )
        self.assertIn("<code>Take point ~= risk / (risk + reward)</code>", result.stdout)
        self.assertNotIn("data-bms-glossary-summary", result.stdout)

    def test_real_lesson_non_highlighted_terms_still_drive_backlinks(self) -> None:
        lessons = learn_glossary.discover_lessons()
        selected = next(
            lesson
            for lesson in lessons
        ...
        self.assertEqual(
            set(selected["terms"]) - set(selected["highlighted_terms"]),
            {
                "take-point",
                "pass",
                "gammon",
        ...
        for slug in selected["terms"]:
            with self.subTest(slug=slug):
                self.assertIn(selected, related[slug])
