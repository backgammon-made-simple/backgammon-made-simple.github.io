...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/release-ui-check.sh [PORT] [--render] [--representative-only]

Runs the repeatable source and rendered-site portion of the BMS UI release
gate. The optional browser phase uses scripts/release_ui_browser_check.mjs
through Codex Browser and is documented in docs/ui-release-testing.md.
...
printf 'Browser procedure: docs/ui-release-testing.md\n'
